...
@EXPORT_OK=qw(
		...
		instructornewenroll
	       ...
	       tweet showtweets showlasttweet seclog showseclog superseclog transactionlog showtranslog

	       equizrender equizgrade equizanswerrender equizrender2 equizrate commentsfortask2table isshared
...
use lib '../..';
use SylSpace::Model::Files qw(eqreadi eqreads longfilename finddue autheqread);
...
sub usernew( $uemail ) {
  ...
  mkdir("$var/users/$uemail/equizzes") or die "cannot create $var/users/$uemail/equizzes: $!\n";
...
sub balance( $uemail ) {
  my $f=bsd_glob("$var/users/$uemail/balance=*");
...
sub transactionlog( $ip, $uemail, $author, $filename, $price ) {
  (userexists($uemail) and userexists($author)) or die "Buyer/Seller does not exist";
  (-e "$var/users/$author/equizzes/$filename") or die "Target file does not belong to Seller";
  ($price > 0) or return;
  _burpapp("$var/users/$uemail/equizzes/transaction.log", $ip."\t".time()."\t".gmtime()."\t".$filename."\t"."-\$$price"."\t".$uemail."\t$author\n");
  _burpapp("$var/users/$author/equizzes/transaction.log", $ip."\t".time()."\t".gmtime()."\t".$filename."\t"."+\$$price"."\t".$uemail."\t$author\n");
}

sub showtranslog( $uemail ) {
  my $translogfile= "$var/users/$uemail/equizzes/transaction.log";
  (-e $translogfile) or return undef;
  return scalar slurp($translogfile);
}
...
sub equizrender( $course, $email, $equizname, $callbackurl ) {
  ...
  my $equizcontent= (isinstructor($course, $email)) ? eqreadi( $course, $equizname ) : eqreads( $course, $equizname, $email);
  ...
  my $fullcommandline= "$executable $fullequizname ask $secret $callbackurl $email free";
...
sub equizrender2 ($author, $equizname, $uemail, $topay, $callbackurl) {
  (_confirmnotdangerous(($equizname =~ s{\~deleted$}{}r), "equizrender")) or die "sorry, but user $uemail cannot post $equizname, because it is bloody";

  (defined($equizname)) or die "need a filename for equizmore.\n";

  my $equizcontent= autheqread($uemail, $equizname, $author);

  my $fullequizname= _getvar()."/users/$author/equizzes/$equizname";
  my $equizlength= length($equizcontent);
  ...
  my $fullcommandline= "$executable $fullequizname ask $secret $callbackurl $uemail $topay";
  ## _confirmnotdangerous($fullcommandline, "executable to render equiz");  ## maybe uncomment
...
sub equizgrade( $course, $uemail, $posttextashash, $decoded=0) {
  sub decryptdecode {
    use Crypt::CBC ;
    use MIME::Base64;
    use HTML::Entities;
  ...
  $decoded or $posttextashash= decodeall($posttextashash);

  my ($conf, $fname, undef, undef, $referrer, $qemail, undef, $time, $browser, $ignoredgradename, $eqlongname)= split(/\|/, $posttextashash->{confidential});
  ...
  (lc($qemail) eq lc($uemail)) or
  (-e "$var/users/$uemail/equizzes/$gradename~$qemail")     # if equiz result is shared 
  or die "Sorry, but $uemail cannot look at answers from $qemail";
  ...
  my $ofname;
  if (($course ne '') and !(isinstructor($course, $uemail))) {
    $ofname= "$var/courses/$course/$uemail/files/$gradename.$time.eanswer.yml";
#    (-e $ofname) and die "please do not answer the same equiz twice.  instead go back, refresh the browser to receive fresh questions, and submit then\n";
    ...
    $ofname= "$var/users/$uemail/equizzes/$gradename.$time.eanswer.yml";
    _safewrite( $posttextashash, $ofname );  ## the content
  }
...
sub equizrate( $ip, $course, $hash ) {
  ...
  my $file = (($course)? '': 'student').'equizratings.txt';
  _logany( $ip, $course, $hash->{equizuemail}, $errmsg, $file, $var );
}

sub commentsfortask2table($course, $task, $author=undef) {
  my @r;
  my ($filepath,$commentpath);
  if ($course eq "") {
    defined($author) or die "Who's the author?";
    $filepath = "$var/users/$author/equizzes/$task";
    $commentpath = "$var/studentequizratings.txt";
  } else { 
    $filepath = "$var/courses/$course/instructor/files/$task";
    $commentpath = "$var/equizratings.txt";
  }
  -e $filepath or die "invalid file path $filepath\n"; 
  -e $commentpath or return undef; 
  open(my $FIN, "<", $commentpath) or return;
  while (<$FIN>) {
    my @c= split(/\t/, $_);
    my @c4= split(/ : /, $c[4]);
    ($c4[0] eq $filepath) or next;
    given($c4[2]) {when ('wrong') {$c4[2]='Answers Seem Wrong';} when ('very') {$c4[2]='Clear Now';} when ('somewhat') {$c4[2]='Still Not Clear';} default {$c4[2]='--';}}
    given($c4[3]) {when ('hard') {$c4[3]='Too Hard';} when ('right') {$c4[3]='Just Right';} when ('easy') {$c4[3]='Too easy';} default {$c4[3]='--';}}
    push(@r, [ $c[3], $c4[2], $c4[3], $c[1], $c4[4] ]);
  }
  return \@r;
}

sub isshared($uemail, $author, $filename) {
print "\n\n\n$var/users/$author/equizzes/$filename\~$uemail";
  return ((-e "$var/users/$author/equizzes/$filename\~$uemail") and (-e "$var/users/$uemail/equizzes/$author\~$filename\~paid"));
}
