...
our @EXPORT_OK=qw(
  ...
  eqwrites eqownlists equnpaidlists eqpaidlists autheqread autheqdelete eqsetprice findprice findauthor 

  finddue ispublic

  _deepsetdue _deeplisti

  longfilename

  cptemplate rmtemplates listtemplates
);#autheqsetdue autheqlisti autheqlists autheqwrite
...
use lib '../..';
use SylSpace::Model::Utils qw( _getvar _confirmsudoset _checksfilenamevalid _checkemailvalid _burpnew _burpapp);
...
sub eqreads( $course, $eqsymname, $uemail='student', $author='instructor') { ($eqsymname=~ /\.equiz[\~]*/) or die "eqsymreads: $eqsymname must end with .equiz"; if (!($author eq 'instructor')) {$eqsymname .= "~author=$author"}; return _basereads( $course, $eqsymname, 1, $uemail); }
...
sub _basereads( $course, $filename, $equizspecial=0, $uemail='student') {
    $course= _checksfilenamevalid($course);
    if ($filename =~ /~author=.*$/) {
      $_ = $filename;
      $_ =~ s/~author=.*$//;
      _checksfilenamevalid($_);
    } else { _checksfilenamevalid($filename); }

  my $lfnm="$var/courses/$course/instructor/files/$filename";
  if (!(($filename =~ /~author=.*$/) or ($filename =~ /~author=$uemail$/)))
...
sub eqwrites( $uemail, $eqsymname, $eqsymcontents, $edit=0) {
  ($eqsymname=~ /\.equiz[\~]*/) or die "eqsymreads: $eqsymname must end with .equiz";
  $uemail= _checkemailvalid($uemail);

  (-e "$var/users/$uemail/") or die "eqwrites: $uemail is not a user";

  _checksfilenamevalid($eqsymname);  ## will die!

  ((-e "$var/users/$uemail/equizzes/$eqsymname") and !($edit)) and die "you already have an uploaded equiz named '$eqsymname'.  please delete first.";

  my $rv= _maybeoverwrite( "$var/users/$uemail/equizzes/$eqsymname", $eqsymcontents );

  touch( "$var/users/$uemail/equizzes/$eqsymname" );

  return $rv;
}

sub eqownlists( $uemail ) {
  $uemail= _checkemailvalid($uemail);
  (-e "$var/users/$uemail/") or die "eqownlists: $uemail is not a user";
  my $ilist= _deeplisti("$var/users/$uemail/equizzes", "*.equiz");
  (rlc($ilist)>0) or return $ilist;
  my @slist; foreach (@{$ilist}) { push(@slist, $_); }  ## always open!
  return \@slist;
}

sub equnpaidlists( $uemail ) {
  $uemail= _checkemailvalid($uemail);
  (-e "$var/users/$uemail/") or die "eqalllists: $uemail is not a user";
  my $ilist= _deeplisti("$var/users", "*/equizzes/*.equiz");
  (rlc($ilist)>0) or return $ilist;
  my @slist; foreach (@{$ilist}) { 
    my $author = $_->{author}; ($author eq $uemail) and next;
    my $filename = %$_{sfilename}; 
    (-e "$var/users/$uemail/equizzes/$author~$filename~paid") and next;
    push(@slist, $_); 
  }
  return \@slist;
}

sub eqpaidlists( $uemail ) {
  $uemail= _checkemailvalid($uemail);
  (-e "$var/users/$uemail/") or die "eqalllists: $uemail is not a user";
  my $ilist= _deeplisti("$var/users", "*/equizzes/*.equiz");
  my $ilistdeleted= _deeplisti("$var/users", q(*/equizzes/*.equiz~deleted));
  (rlc($ilist)>0 or rlc($ilistdeleted)>0) or return $ilist;
  my @slist; 
  $ilist = [@{$ilist},@{$ilistdeleted}];

  foreach (@{$ilist}) { 
    my $author = $_->{author}; ($author eq $uemail) and next;
    my $filename = %$_{sfilename}; $filename =~ s{\~deleted}{};
    (-e "$var/users/$uemail/equizzes/$author\~$filename\~paid") or next;
    push(@slist, $_); 
  }
...
sub autheqread ($uemail, $eqsymname, $author) {
  _checksfilenamevalid(($eqsymname =~ s{\~deleted$}{}r));

  my $lfnm="$var/users/$author/equizzes/$eqsymname";
  if (($author ne $uemail) and !(-e "$var/users/$uemail/equizzes/$author\~".($eqsymname =~ s{\~deleted$}{}r)."\~paid"))
     { (time() >= finddue( $lfnm )) and die "sorry, but there is no (longer) $lfnm, or it's never published"; }

  return slurp $lfnm;
}

sub autheqdelete ( $author, $sfilename ) {
  $sfilename =~ s{$var/users/$author/equizzes/}{};
  _checksfilenamevalid($sfilename);
  my $lfilename = "$var/users/$author/equizzes/$sfilename";
  (-e $lfilename) or die "cannot delete non-existing $lfilename";

  my @duefile = bsd_glob("$lfilename\~*");
  my $pricefile = bsd_glob("$lfilename\~price=*");
  foreach (@duefile) { ($_ ne "$lfilename~grades") and ($_ ne $pricefile) and unlink($_); } 

  rename($lfilename,"$lfilename~deleted") or die "cannot rename existing file $lfilename to $lfilename~deleted\n";
  (-e "$lfilename~grades") and (rename("$lfilename~grades","$lfilename~deleted~grades") or die "cannot rename existing file $lfilename~grades to $lfilename~deleted~grades\n");
  if (-e $pricefile) {
    my $newpricefile = $pricefile; $newpricefile =~ s/\~price=/\~deleted\~price=/;
    rename($pricefile,$newpricefile) or die "cannot rename existing file $pricefile to $newpricefile\n";
  }
  _burpapp("$lfilename~deleted","\n"); ## just to modify it a bit to update mtime
}

sub eqsetprice($uemail, $eqsymname, $author, $newprice) {
  _checksfilenamevalid($eqsymname);

  my $lfnm="$var/users/$author/equizzes/$eqsymname";
  ($author eq $uemail) or die "You can't change the price of $eqsymname";
  -e $lfnm or die "$eqsymname does not exist";
  (defined($newprice) and $newprice>=0) or die "What's your new price? It can't be negative!";
  foreach (bsd_glob("$lfnm\~price=*")) { unlink($_); }
  touch("$lfnm\~price=$newprice");
}

sub findprice($lfilename) {
  ((-l $lfilename) || (-e $lfilename)) or die "cannot read due date for non-existing file $lfilename";

  my @pricelist= bsd_glob("$lfilename\~price=*");
  (@pricelist) or return 0;
  (my $f= $pricelist[0]) =~ s{.*\~price=}{};
  ($f =~ m/^-/) and die "invalid negative price $f for $lfilename";
  (looks_like_number($f)) or die "invalid price $f for $lfilename";
  return $f;
}

sub findauthor($lfilename) {
  ((-l $lfilename) || (-e $lfilename)) or die "cannot read due date for non-existing file $lfilename";
  ($lfilename =~ m{^$var/users/.*/equizzes/.*}) or return undef;
  $lfilename =~ s{^$var/users/}{};
  $lfilename =~ s{/equizzes/.*}{};
  _checkemailvalid($lfilename);
  return $lfilename;
}
################################################################
## TEMPLATES
...
sub _deeplisti( $globdir, $globfilename ) {
  ...
  foreach (bsd_glob("$globdir/$globfilename")) {
    my %parms;
    ($_ =~ /\~/) and (($_ =~ /\~deleted$/) or next);  ## these are meta files!
    if ($nothwequiz) {
      ($_ =~ /\.equiz$/) and next;
    ...
    $parms{price}= findprice($_);
    $parms{author}= findauthor($_);
    (_findanswer($_)) and $parms{answer}= _findanswer($_);
    if ($_ =~ m{.equiz(~deleted)*}) {
      my @matches = ((slurp $_) =~ /\:A\:/g); my $count = @matches;
      $parms{numofquestions}= $count;
    }
    push(@filelist, \%parms);
  }
  return \@filelist;
