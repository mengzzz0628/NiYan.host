...
sub renderequiz {
  ...
  my ($equizfilename, $mode, $secret, $callbackurl, $user, $paid) = @_;  ## just pass back
  my $time= time();
...
# name* instructor* gradename area subarea license created version render comment intro ps comment eqversion sharing paging email shuffle finish_page
  ...
  foreach my $q (@{$qz->{q}}) {
	...
	.(defined($q->{'C'}) ? drawinputmultchoice($qcnt, $q->{'C'},$paid) : 
	((defined($paid) and ($paid eq "unpaid"))? "" : drawinputtextfield($qcnt)))
  ...
  my $template= q(
    ...
    <div class="equiz">
      ...
      <div class="container">
        ...
        <form method="post" class="quizform" action=)
  .((defined($paid) and ($paid eq "unpaid"))? q("/payequiz?toequiz=).$equizfilename.q(&from=).$user.q(") : q("[+ $callbackurl +]")).q(>
         ...
         <input type="hidden" name="ntime" value="[+ $ntime +]" />
           ...
           <div class="quizsubmitbutton">
              <input type="submit" class="quizsubmitbutton" name="submit" value=")
  .((defined($paid) and ($paid eq "unpaid"))? q(Pay to grade your answer) : q(Submit and Grade my Answers))
  .q(" /><br />
...
sub drawinputmultchoice {
    my ($qcntname,$choices,$paid)= @_;

    my @choices= split(/\|/, $choices);
    my $rv= qq(\t\t<fieldset> <legend>Please Choose One</legend>\n\t\t<ol class="eqchoice" id="MC$qcntname">\n);
    for (my $i=0; $i<= $#choices; ++$i) {
      $rv .= qq(\t\t<li> <input class="eqchoice foo" type="radio").((defined($paid) and ($paid eq "unpaid"))? q( disabled="disabled" ): "").q( required value=").($i+1).qq(" name=\"q-stdnt-$qcntname\" />&nbsp;&nbsp; $choices[$i] </li>\n);
    }
    $rv .= "\t\t\t</ol>\n\t\t\t</fieldset>\n";
    return $rv;
  }
