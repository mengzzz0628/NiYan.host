...
($equizfilename =~ /\.equiz(\~deleted)*$/i) or die "sorry, your file must end with extension 'equiz', not $equizfilename.";
...
if ($mode =~ /ask/) {
  ($#ARGV >= 4) or die "not enough arguments for mode 'ask'/'normal':\n$usage";
  ...
  my $author= $equizfilename; $author =~ s{.*/users/}{};
  my $filename= $author; $filename=~ s{.*/equizzes/}{}; $author =~ s{/equizzes/.*$}{};
  $ARGV[2] .= "?f=$filename&author=$author";
