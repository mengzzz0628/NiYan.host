...
sub drawmore($sfilename, $centertype, $actionchoices, $allfiledetails, $tzi, $webbrowser="",$author='instructor') {

  my $detail;
  foreach (@{$allfiledetails}) {
    ## we need to ferret the creation and due times
    if ($_ -> {sfilename} eq $sfilename) { (defined($_->{author}) and ($_->{author} ne $author)) and next; $detail= $_; last; }
  }

  my $fname= $detail->{sfilename};
  
  my $curprice= $detail->{price} || 0;
  my $price= ($author eq 'instructor')? "" : "<tr> <th> price </th> <td>".( (scalar @$actionchoices >= 3)?  
		     qq(<form method="get" action="equizsetprice?f=$fname&author=$author" class="form-inline">
			<input type="hidden" name="f" value="$fname" />
			<input type="hidden" name="author" value="$author" />
			Set price to: <input required type="number" id="price" name="price" placeholder="0.00" value="$curprice" min="0" step="0.01" onblur="submit();" />
			<input type="submit" id="submit" value="update" class="btn btn-xs btn-default" />
			</form>) : "\$$curprice")." </td></tr>";

  my $achoices= actionchoices( $actionchoices, $fname, $author);

  my $changedtime= _epochfour( $detail->{mtime}||0, $tzi );
  my $delbutton= btn("filedelete?f=$fname".(($author eq 'instructor')? "" : "&author=$author"), 'delete', 'btn-xs btn-danger');
  my $backbutton= ($author eq 'instructor')? btn($centertype."center", "back to ${centertype}center", 'btn-xs btn-default') : btn('goclass','back','btn-xs btn-default');
  ...
  $webbrowser = ((defined($webbrowser)) and ($webbrowser eq 'safari')) ? qq(<br />safari's date selector is broken.  please complain to apple and use chrome<br />until then, use mm/dd/yyyy) : '';

  my $sixmobutton= "<p style=\"padding:1em\"> or <span style=\"padding:2ex\">".btn("filesetdue?f=$fname&author=$author&amp;dueepoch=".(time()+24*3600*180), "publish for 6 Months", 'btn-default')."</span>".
     " or <span style=\"padding:2ex\">".btn("filesetdue?f=$fname&author=$author&amp;dueepoch=".(time()-2), "unpublish", 'btn-default')."</span></p>";
  ...
  <table class="table">
	...
	<tr> <th> visible until </th> <td> $duetimefour
			<p>
			).((scalar @$actionchoices >= 3)? qq(<form method="get" action="filesetdue?f=$fname&author=$author" class="form-inline">
			<input type="hidden" name="f" value="$fname" />
			<input type="hidden" name="author" value="$author" />
	...
	<tr> <th> delete </th> <td> $delbutton) : '').qq( </td> </tr>
...
sub ifilehash2table( $filehashptr, $actionchoices, $type, $tzi, $author='instructor') {
...
sub actionchoices( $actionchoices, $fname, $author='instructor') {
  my $selector= {
		 equizrun => ($author eq 'instructor')? btn("/equizrender?f=$fname", 'run', 'btn-xs btn-default'):btn("/equizrender2?f=$fname&author=$author", 'run', 'btn-xs btn-default'),
