...
get '/equizrate' => sub {
  my $c = shift;
  (my $course = standard( $c )) or ($c->req->url->subdomain eq 'auth') or return global_redirect($c);
  ...
  $c->flash(message => "Thank you for your equiz rating")->redirect_to(($c->req->url->subdomain eq 'auth')? "/auth/goclass": "/student/equizcenter");
};
