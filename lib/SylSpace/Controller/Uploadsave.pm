...
post '/uploadsave' => sub {
  ...
  if (isinstructor( $course, $c->session->{uemail})) {
    ...
    if ($hwmatch eq 'nothing yet') { #or die "uploadsave error: what hw are you trying to answer??";
      my $result= eval { eqwrites($course,$c->session->{uemail},$filename, $filecontents) };
      $@ and die "Problem $result Writing Equiz : '$@'  Course: $course
Uemail: ".($c->session->{uemail})."<br />
Filename: $filename
Filecontents: ".length($filecontents)." bytes.
</pre>
 ";
      $referto= "/student/equizcenter";
    } else {
